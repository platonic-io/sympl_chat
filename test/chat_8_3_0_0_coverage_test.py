...
class TestChatCoverage():
    def test_create_room(self, chat_8, store):
    ...
    def test_room_visibility(self, store, chat_8):
    ...
    def test_cannot_get_messages_from_deleted_room(self, store, chat_8):
    ...
    def test_deleted_room_not_included_in_get_rooms(self, store, chat_8):
        """Once a room is deleted, no user can get messages from it."""
        create_room_event = chat_8('alice').create_room(room_name='room')
        room = create_room_event['room']['channel']
        chat_8('alice').delete_room(room_channel=room)
    ...
    def test_restore_room(self, store, chat_8):
    ...
    def test_get_rooms(self, store, chat_8):
        chat_8('alice').create_room(room_name='room_1')
    ...
    def test_get_rooms_sorted_by_name(self, store, chat_8):
        ...
        chat_8('alice').create_room(room_name='room_1')
        chat_8('alice').create_room(room_name='room_2')
        chat_8('alice').create_room(room_name='room_3')
        chat_8('alice').create_room(room_name='room_4')
        chat_8('alice').create_room(room_name='room_5')
        rooms = chat_8('alice').get_rooms()
        assert rooms == sorted(rooms, key=lambda r: r['name'])

    def test_no_messages_over_4000_charts(self, store, chat_8):
        room = chat_8('alice').create_room(room_name='room')['room']['channel']
        message = "m" * 4001
        with pytest.raises(ContractError) as e:
            chat_8('alice').send_message(room_channel=room, message=message)
