...
class TestChatCoverage():
    def test_create_room(self, chat_9, store):
        """When a user creates a room, they are able to send messages, get messages, and invite more users."""
        create_room_event = chat_9('alice').create_room(room_name='room')
        room = create_room_event['room']['channel']
        chat_9('alice').get_messages(room_channel=room)
    ...
    def test_cannot_get_messages_from_deleted_room(self, store, chat_9):
    ...
    def test_deleted_room_not_included_in_get_rooms(self, store, chat_9):
        """Once a room is deleted, no user can get messages from it."""
        create_room_event = chat_9('alice').create_room(room_name='room')
        room = create_room_event['room']['channel']
        chat_9('alice').delete_room(room_channel=room)
        rooms = chat_9('alice').get_rooms()
        assert (room not in [room['channel'] for room in rooms])

    def test_restore_room(self, store, chat_9):
    ...
    def test_get_rooms(self, store, chat_9):
        chat_9('alice').create_room(room_name='room_1')
        chat_9('alice').create_room(room_name='room_2')
        rooms = chat_9('alice').get_rooms()
    ...
    def test_get_rooms_change_after_person_left_and_promote_owner(self, network, store, chat_9):
        store['eve'] = network.register_key_alias()
        create_room_event = chat_9('alice').create_room(room_name='room')
        room=create_room_event["room"]["channel"]
        chat_9('alice').invite_to_room(new_member=store['bob'], room_channel=room)
        chat_9('alice').invite_to_room(new_member=store['eve'], room_channel=room)
        chat_9('alice').remove_from_room(member_to_remove=store['bob'], room_channel=room)
        chat_9('alice').promote_to_owner(member=store['eve'], room_channel=room)
        alice_rooms = chat_9('alice').get_rooms()
        bob_rooms = chat_9('bob').get_rooms()
    ...
    def test_demote_owner(self, store, chat_9):
        create_room_event = chat_9('alice').create_room(room_name='room')
        room=create_room_event["room"]["channel"]
        chat_9('alice').invite_to_room(new_member=store['bob'], room_channel=room)
        chat_9('alice').promote_to_owner(member=store['bob'], room_channel=room)
        rooms = chat_9('alice').get_rooms()
        utils.scrub_channels(rooms)
        assert rooms == [{'name': 'room', 'is_deleted': False, 'members': [store['alice'], store['bob']], 'owners':[store['alice'], store['bob']]}]
        chat_9('alice').demote_owner(owner=store['bob'], room_channel=room)
        rooms = chat_9('alice').get_rooms()
        utils.scrub_channels(rooms)
        assert rooms == [{'name': 'room', 'is_deleted': False, 'members': [store['alice'], store['bob']], 'owners':[store['alice']]}]

    def test_get_rooms_sorted_by_name(self, store, chat_9):
        chat_9('alice').create_room(room_name='room_0')
        chat_9('alice').create_room(room_name='room_1')
        chat_9('alice').create_room(room_name='room_2')
        chat_9('alice').create_room(room_name='room_3')
        chat_9('alice').create_room(room_name='room_4')
        chat_9('alice').create_room(room_name='room_5')
        rooms = chat_9('alice').get_rooms()
        assert rooms == sorted(rooms, key=lambda r: r['name'])

    def test_no_messages_over_4000_charts(self, store, chat_9):
        room = chat_9('alice').create_room(room_name='room')['room']['channel']
        message = "m" * 4001
        with pytest.raises(ContractError) as e:
            chat_9('alice').send_message(room_channel=room, message=message)
